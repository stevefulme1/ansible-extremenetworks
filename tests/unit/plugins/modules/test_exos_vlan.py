"""Unit tests for stevefulme1.extremenetworks.exos_vlan module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.modules.exos_vlan"
CLIENT_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for exos_vlan."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"vlan_id": "res-123", "name": "test-exos_vlan"}
    client.update.return_value = {"vlan_id": "res-123", "name": "test-exos_vlan-updated"}
...
def existing_resource():
    """Return a dict representing an existing exos_vlan."""
    return {
        "vlan_id": "res-123",
        "name": "test-exos_vlan",
        "state": "active",
...
class TestCreateExosVlan:
    """Tests for creating a exos_vlan."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("exos_vlan", {"name": "test-exos_vlan"})
        assert result["vlan_id"] == "res-123"
        assert result["name"] == "test-exos_vlan"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("exos_vlan", params)
        mock_api_client.create.assert_called_once_with("exos_vlan", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("exos_vlan", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "exos_vlan": {}}
        else:
            result = mock_api_client.create("exos_vlan", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateExosVlan:
    """Tests for updating a exos_vlan."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("exos_vlan", "res-123", {"name": "updated"})
        assert result["name"] == "test-exos_vlan-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("exos_vlan", "bad-id", {})


class TestDeleteExosVlan:
    """Tests for deleting a exos_vlan."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("exos_vlan", "res-123")
        mock_api_client.delete.assert_called_once_with("exos_vlan", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("exos_vlan", "missing-id")
...
class TestGetExosVlan:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("exos_vlan", "res-123")


class TestListExosVlan:
    """Tests for listing exos_vlan resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"vlan_id": "1", "name": "first"},
            {"vlan_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("exos_vlan")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"vlan_id": "1", "name": "match"}]
        result = mock_api_client.list("exos_vlan", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyExosVlan:
    """Tests for idempotent behavior of exos_vlan."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("exos_vlan", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("exos_vlan", "missing") is not None
        assert exists is False


class TestErrorHandlingExosVlan:
    """Tests for error handling in exos_vlan."""
