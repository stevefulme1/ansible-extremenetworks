"""Unit tests for stevefulme1.extremenetworks.exos_acl module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.modules.exos_acl"
CLIENT_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for exos_acl."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"acl_name": "res-123", "acl_name": "test-exos_acl"}
    client.update.return_value = {"acl_name": "res-123", "acl_name": "test-exos_acl-updated"}
...
def existing_resource():
    """Return a dict representing an existing exos_acl."""
    return {
        "acl_name": "res-123",
        "acl_name": "test-exos_acl",
        "state": "active",
    }


class TestCreateExosAcl:
    """Tests for creating a exos_acl."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("exos_acl", {"acl_name": "test-exos_acl"})
        assert result["acl_name"] == "res-123"
        assert result["acl_name"] == "test-exos_acl"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "acl_name": "full-exos_acl",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("exos_acl", params)
        mock_api_client.create.assert_called_once_with("exos_acl", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("exos_acl", {"acl_name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "exos_acl": {}}
        else:
            result = mock_api_client.create("exos_acl", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateExosAcl:
    """Tests for updating a exos_acl."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("exos_acl", "res-123", {"acl_name": "updated"})
        assert result["acl_name"] == "test-exos_acl-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"acl_name": existing_resource["acl_name"]}
        current = {"acl_name": existing_resource["acl_name"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("exos_acl", "bad-id", {})


class TestDeleteExosAcl:
    """Tests for deleting a exos_acl."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("exos_acl", "res-123")
        mock_api_client.delete.assert_called_once_with("exos_acl", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("exos_acl", "missing-id")
...
class TestGetExosAcl:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("exos_acl", "res-123")


class TestListExosAcl:
    """Tests for listing exos_acl resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"acl_name": "1", "acl_name": "first"},
            {"acl_name": "2", "acl_name": "second"},
        ]
        result = mock_api_client.list("exos_acl")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"acl_name": "1", "acl_name": "match"}]
        result = mock_api_client.list("exos_acl", filters={"acl_name": "match"})
        assert len(result) == 1


class TestIdempotencyExosAcl:
    """Tests for idempotent behavior of exos_acl."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("exos_acl", "res-123")
        desired_params = {"acl_name": current["acl_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["acl_name"] != current["acl_name"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("exos_acl", "missing") is not None
        assert exists is False


class TestErrorHandlingExosAcl:
    """Tests for error handling in exos_acl."""
