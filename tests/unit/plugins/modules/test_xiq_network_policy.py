"""Unit tests for stevefulme1.extremenetworks.xiq_network_policy module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.modules.xiq_network_policy"
CLIENT_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for xiq_network_policy."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"policy_id": "res-123", "name": "test-xiq_network_policy"}
    client.update.return_value = {"policy_id": "res-123", "name": "test-xiq_network_policy-updated"}
...
def existing_resource():
    """Return a dict representing an existing xiq_network_policy."""
    return {
        "policy_id": "res-123",
        "name": "test-xiq_network_policy",
        "state": "active",
    }


class TestCreateXiqNetworkPolicy:
    """Tests for creating a xiq_network_policy."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("xiq_network_policy", {"name": "test-xiq_network_policy"})
        assert result["policy_id"] == "res-123"
        assert result["name"] == "test-xiq_network_policy"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-xiq_network_policy",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("xiq_network_policy", params)
        mock_api_client.create.assert_called_once_with("xiq_network_policy", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("xiq_network_policy", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "xiq_network_policy": {}}
        else:
            result = mock_api_client.create("xiq_network_policy", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateXiqNetworkPolicy:
    """Tests for updating a xiq_network_policy."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("xiq_network_policy", "res-123", {"name": "updated"})
        assert result["name"] == "test-xiq_network_policy-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("xiq_network_policy", "bad-id", {})


class TestDeleteXiqNetworkPolicy:
    """Tests for deleting a xiq_network_policy."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("xiq_network_policy", "res-123")
        mock_api_client.delete.assert_called_once_with("xiq_network_policy", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("xiq_network_policy", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("xiq_network_policy", "res-123")


class TestGetXiqNetworkPolicy:
    """Tests for getting a xiq_network_policy."""

    def test_get_existing(self, mock_api_client, existing_resource):
        """Verify get returns resource when it exists."""
        mock_api_client.get.return_value = existing_resource
        result = mock_api_client.get("xiq_network_policy", "res-123")
        assert result["policy_id"] == "res-123"

    def test_get_nonexistent(self, mock_api_client):
        """Verify get returns None for missing resource."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("xiq_network_policy", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("xiq_network_policy", "res-123")


class TestListXiqNetworkPolicy:
    """Tests for listing xiq_network_policy resources."""
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        result = mock_api_client.list("xiq_network_policy")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        result = mock_api_client.list("xiq_network_policy", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyXiqNetworkPolicy:
    """Tests for idempotent behavior of xiq_network_policy."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("xiq_network_policy", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("xiq_network_policy", "missing") is not None
        assert exists is False


class TestErrorHandlingXiqNetworkPolicy:
    """Tests for error handling in xiq_network_policy."""
    ...
    def test_auth_failure(self):
        ...
        with pytest.raises(Exception, match="401 Unauthorized"):
            client.create("xiq_network_policy", {})
    ...
    def test_rate_limit(self):
        ...
        with pytest.raises(Exception, match="429"):
            client.list("xiq_network_policy")
