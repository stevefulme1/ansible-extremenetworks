"""Unit tests for stevefulme1.extremenetworks.exos_bgp module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.modules.exos_bgp"
...
class TestCreateExosBgp:
    """Tests for creating a exos_bgp."""
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        params = {
            "neighbor_address": "full-exos_bgp",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("exos_bgp", params)
        mock_api_client.create.assert_called_once_with("exos_bgp", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("exos_bgp", {"neighbor_address": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "exos_bgp": {}}
        else:
            result = mock_api_client.create("exos_bgp", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateExosBgp:
    """Tests for updating a exos_bgp."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("exos_bgp", "res-123", {"neighbor_address": "updated"})
        assert result["neighbor_address"] == "test-exos_bgp-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"neighbor_address": existing_resource["neighbor_address"]}
        current = {"neighbor_address": existing_resource["neighbor_address"]}
    ...
    def test_update_detects_changes(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        desired = {"neighbor_address": "new-name"}
        current = {"neighbor_address": existing_resource["neighbor_address"]}
        changed = desired != current
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("exos_bgp", "bad-id", {})


class TestDeleteExosBgp:
    """Tests for deleting a exos_bgp."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("exos_bgp", "res-123")
        mock_api_client.delete.assert_called_once_with("exos_bgp", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("exos_bgp", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("exos_bgp", "res-123")


class TestGetExosBgp:
    """Tests for getting a exos_bgp."""
    ...
    def test_get_nonexistent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        result = mock_api_client.get("exos_bgp", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("exos_bgp", "res-123")


class TestListExosBgp:
    """Tests for listing exos_bgp resources."""
...
class TestIdempotencyExosBgp:
    """Tests for idempotent behavior of exos_bgp."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("exos_bgp", "res-123")
        desired_params = {"neighbor_address": current["neighbor_address"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["neighbor_address"] != current["neighbor_address"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("exos_bgp", "missing") is not None
        assert exists is False


class TestErrorHandlingExosBgp:
    """Tests for error handling in exos_bgp."""
