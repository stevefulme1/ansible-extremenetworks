"""Unit tests for stevefulme1.extremenetworks.voss_vlan module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.modules.voss_vlan"
CLIENT_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for voss_vlan."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"vlan_id": "res-123", "name": "test-voss_vlan"}
    client.update.return_value = {"vlan_id": "res-123", "name": "test-voss_vlan-updated"}
...
def existing_resource():
    """Return a dict representing an existing voss_vlan."""
    return {
        "vlan_id": "res-123",
        "name": "test-voss_vlan",
        "state": "active",
...
class TestCreateVossVlan:
    """Tests for creating a voss_vlan."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("voss_vlan", {"name": "test-voss_vlan"})
        assert result["vlan_id"] == "res-123"
        assert result["name"] == "test-voss_vlan"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("voss_vlan", params)
        mock_api_client.create.assert_called_once_with("voss_vlan", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("voss_vlan", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "voss_vlan": {}}
        else:
            result = mock_api_client.create("voss_vlan", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateVossVlan:
    """Tests for updating a voss_vlan."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("voss_vlan", "res-123", {"name": "updated"})
        assert result["name"] == "test-voss_vlan-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("voss_vlan", "bad-id", {})


class TestDeleteVossVlan:
    """Tests for deleting a voss_vlan."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("voss_vlan", "res-123")
        mock_api_client.delete.assert_called_once_with("voss_vlan", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("voss_vlan", "missing-id")
...
class TestGetVossVlan:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("voss_vlan", "res-123")


class TestListVossVlan:
    """Tests for listing voss_vlan resources."""
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        result = mock_api_client.list("voss_vlan")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        result = mock_api_client.list("voss_vlan", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyVossVlan:
    """Tests for idempotent behavior of voss_vlan."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("voss_vlan", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("voss_vlan", "missing") is not None
        assert exists is False


class TestErrorHandlingVossVlan:
    """Tests for error handling in voss_vlan."""
