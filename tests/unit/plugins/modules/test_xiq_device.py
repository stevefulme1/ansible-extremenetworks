"""Unit tests for stevefulme1.extremenetworks.xiq_device module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.modules.xiq_device"
CLIENT_PATH = "ansible_collections.stevefulme1.extremenetworks.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for xiq_device."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"device_id": "res-123", "hostname": "test-xiq_device"}
    client.update.return_value = {"device_id": "res-123", "hostname": "test-xiq_device-updated"}
...
def existing_resource():
    """Return a dict representing an existing xiq_device."""
    return {
        "device_id": "res-123",
        "hostname": "test-xiq_device",
        "state": "active",
    }


class TestCreateXiqDevice:
    """Tests for creating a xiq_device."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("xiq_device", {"hostname": "test-xiq_device"})
        assert result["device_id"] == "res-123"
        assert result["hostname"] == "test-xiq_device"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "hostname": "full-xiq_device",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("xiq_device", params)
        mock_api_client.create.assert_called_once_with("xiq_device", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("xiq_device", {"hostname": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "xiq_device": {}}
        else:
            result = mock_api_client.create("xiq_device", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateXiqDevice:
    """Tests for updating a xiq_device."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("xiq_device", "res-123", {"hostname": "updated"})
        assert result["hostname"] == "test-xiq_device-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"hostname": existing_resource["hostname"]}
        current = {"hostname": existing_resource["hostname"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("xiq_device", "bad-id", {})


class TestDeleteXiqDevice:
    """Tests for deleting a xiq_device."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("xiq_device", "res-123")
        mock_api_client.delete.assert_called_once_with("xiq_device", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("xiq_device", "missing-id")
...
class TestGetXiqDevice:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("xiq_device", "res-123")


class TestListXiqDevice:
    """Tests for listing xiq_device resources."""
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        result = mock_api_client.list("xiq_device")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"device_id": "1", "hostname": "match"}]
        result = mock_api_client.list("xiq_device", filters={"hostname": "match"})
        assert len(result) == 1


class TestIdempotencyXiqDevice:
    """Tests for idempotent behavior of xiq_device."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("xiq_device", "res-123")
        desired_params = {"hostname": current["hostname"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["hostname"] != current["hostname"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("xiq_device", "missing") is not None
        assert exists is False


class TestErrorHandlingXiqDevice:
    """Tests for error handling in xiq_device."""
