...
EXAMPLES = r"""
- name: List all acls
  stevefulme1.extremenetworks.exos_acl_info:
    host: api.example.com
  register: result

- name: Get a specific acl
  stevefulme1.extremenetworks.exos_acl_info:
    host: api.example.com
    acl_name: "example-id"
  register: result
...
def main():
    ...
    if resource_id:
        result = client.get("acl", resource_id)
        resources = [result] if result else []
    else:
        resources = client.list("acl", module.params)

    module.exit_json(changed=False, acls=resources)
