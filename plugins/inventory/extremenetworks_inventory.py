...
options:
    host:
        ...
        required: true
        env:
            - name: EXTREMENETWORKS_HOST
    api_key:
        description: API key for authentication.
        type: str
        env:
            - name: EXTREMENETWORKS_API_KEY
    username:
        description: Authentication username.
        type: str
        env:
            - name: EXTREMENETWORKS_USERNAME
    password:
        description: Authentication password.
        type: str
        env:
            - name: EXTREMENETWORKS_PASSWORD
    validate_certs:
...
EXAMPLES = r"""
# extremenetworks_inventory.yml
plugin: stevefulme1.extremenetworks.extremenetworks_inventory
host: api.example.com
api_key: "{{ lookup('env', 'EXTREMENETWORKS_API_KEY') }}"
"""
...
class InventoryModule(BaseInventoryPlugin):
    NAME = "stevefulme1.extremenetworks.extremenetworks_inventory"
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        group = self.inventory.add_group("extremenetworks")
        ...
        try:
            ...
            for item in resp.json().get("data", []):
                ...
                if hostname:
                    self.inventory.add_host(hostname, group="extremenetworks")
